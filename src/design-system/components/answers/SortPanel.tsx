            ...
            {unassignedItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-sm font-bold transition-all cursor-pointer border text-left max-w-full ${
                    isSelected
              ...
              <div className="flex flex-wrap gap-1.5 flex-1 items-start">
                {bucketItems.length === 0 ? (
                  <span className="text-xs text-slate-500 italic py-2">
                    {isTarget ? 'Nhấn vào đây để xếp vào nhóm' : 'Chưa có mục nào'}
                  </span>
                ) : (
                  bucketItems.map((item) => (
                    <span
                      key={item.id}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs font-semibold text-slate-200 max-w-full text-left"
                    >
                      ...
                      {!disabled && (
                        <button
                          ...
                          className="hover:text-rose-400 transition-colors cursor-pointer ml-0.5 shrink-0"
                        >
